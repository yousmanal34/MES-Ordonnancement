...
def flowshop_scheduling_with_objective_no_wait(processing_times, release_times, deadlines, M):
    ...
    # No-wait constraint: Le job k commence immédiatement sur la machine m dès qu'il est terminé sur la machine précédente
    for m in range(1, num_machines):  # à partir de la deuxième machine
        for k in range(num_jobs):
            model.addConstr(start[k, m] == c[k, m - 1])  # Le job k commence sur la machine m dès qu'il termine sur m-1
